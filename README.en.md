...
2. Read the game display version, Steam buildid, depot manifest, and exe/UE version, and keep those signals separate.
3. Query SteamDB with fixed app id `4704690`, package, and depot pages for public app/package/depot update evidence, then compare it with the version declared in the upstream README/releases/latest commits.
...
The workflow keeps these version signals separate: `2.5.0` / `2.5.1` style values are player-facing game versions and should only be compared directly with an author's declared Game Version; Steam `buildid` is the locally installed package build, for example game display version `2.5.1` may correspond to `buildid 24089838`; the depot manifest is a more precise Steam package-content signal; SteamDB app/package/depot timestamps show whether the public Steam package changed; and `++UE5+Release-...` is the engine/exe version. Route selection primarily compares the game display version with the upstream declaration, then uses local buildid, depot manifest, and SteamDB timestamps to catch same-version silent package updates.
